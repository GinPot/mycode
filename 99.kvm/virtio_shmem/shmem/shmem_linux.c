...
static int major,perform_major;
...
//ljm for display sharing

static struct shmem_data *  display_sharing_data;
...
static hyp_fbinfo_t * hyp_fb[2];

struct input_dev *keyinput;
//static int perform_idx;
static struct class  *perform_class;
static struct device *perform_dev;
char *performshm, *qnxdatatmp;
struct TopProce {
	char name[25];
	int total;
};
struct AglBuff {
	int cpu_load;
	int memoryload[2];
	struct TopProce appdata[5];
};
struct AglBuff *liagldata;
...
shmem_print_incoming(unsigned long arg)
{
    ...
    for (i = 0; i < GUEST_SHM_MAX_CLIENTS; ++i) {
        ...
        if (incoming & (1u << i)) {
			...
			if((data->shmdata[i*BUFF_SIZE] == 0xA1) && (data->shmdata[i*BUFF_SIZE+1] == 0x1A))
			{
				//dev_info(dev, "KEY VAL = %d", data->shmdata[i*BUFF_SIZE+2]);
				input_report_key(keyinput ,  (data->shmdata[i*BUFF_SIZE+2]) , (data->shmdata[i*BUFF_SIZE+3]));
				input_sync(keyinput);
			}
			
			if((data->shmdata[i*BUFF_SIZE] == 0x45) && (data->shmdata[i*BUFF_SIZE+1] == 0x54))
			{
				//perform_idx = i;
				//dev_info(dev, "performancedisplay idx %d", perform_idx);
				performshm = &data->shmdata[i*BUFF_SIZE];
			}			
        }
    }
...
int perform_open (struct inode *inode, struct file *file)
{
	
	return 0;
}

ssize_t perform_write (struct file *file, const char __user *buf, size_t size, loff_t *ppos)
{
	int ret,i,len;

	ret = copy_from_user(qnxdatatmp, buf, size);

	for(i=0; i<size; i++)
		performshm[i] = qnxdatatmp[i];

	return size;
}

static struct file_operations perform_fops = {
...
shmem_probe(struct device * const dev, volatile struct guest_shm_factory * const factory, int irq)
{
	...
	liagldata = kzalloc(sizeof(struct AglBuff), GFP_KERNEL);
    if (liagldata == NULL) {
        dev_err(dev, "no memory for liagldata");
        return -ENOMEM;
    }
	qnxdatatmp = (char *)liagldata;
    ...
    //ljm display sharing
    display_sharing_data = data;
    hyp_fb[0] = (hyp_fbinfo_t *) display_sharing_data->mybuff;
    hyp_fb[1] = (hyp_fbinfo_t *) (display_sharing_data->mybuff +sizeof(hyp_fbinfo_t));
    printk("ljm hyp_fb[0] addr %p, hyp_fb[1] addr %p\n",hyp_fb[0],hyp_fb[1]);
    hyp_fb[0]->vaddr = NULL;
    hyp_fb[0]->paddr = 9999;
    hyp_fb[0]->fbid = 9999;
    hyp_fb[0]->handle = 9999;
    hyp_fb[0]->owner = 9999;
    hyp_fb[0]->isfront = 9999;

    hyp_fb[1]->vaddr = NULL;
    hyp_fb[1]->paddr = 9999;
    hyp_fb[1]->fbid = 9999;
    hyp_fb[1]->handle = 9999;
    hyp_fb[1]->owner = 9999;
    hyp_fb[1]->isfront = 9999;
	...
	keyinput = input_allocate_device();
	keyinput->name = "key_input";
	keyinput->phys = "ads1015";

	keyinput->evbit[0] = BIT_MASK(EV_KEY);
	__set_bit(EV_KEY, keyinput->evbit);					//添加按键事件
	__set_bit(EV_SYN, keyinput->evbit);					//添加同步事件
	__set_bit(KEY_UP, keyinput->keybit);				//添加键盘值
	__set_bit(KEY_DOWN, keyinput->keybit);
	
	__set_bit(KEY_MUTE, keyinput->keybit);
	__set_bit(KEY_LEFT, keyinput->keybit);
	__set_bit(KEY_RIGHT, keyinput->keybit);
	
	__set_bit(KEY_PHONE, keyinput->keybit);
	__set_bit(KEY_RECORD, keyinput->keybit);
	__set_bit(KEY_STOP, keyinput->keybit);
	
	ret = input_register_device(keyinput);
	if (ret) {
		input_free_device(keyinput);
		return ret;
	}

	//创建新能数据采集的设备节点
	perform_major = register_chrdev(0, "dev_perform", &perform_fops);
	perform_class = class_create(THIS_MODULE, "dev_perform");
	/* 为了让mdev根据这些信息来创建设备节点 */
	perform_dev = device_create(perform_class, NULL, MKDEV(perform_major, 0), NULL, "dev_perform"); /* /dev/buttons */
...
shmem_remove(struct device * const dev)
{
	...
	unregister_chrdev(major, "dev_perform");
	device_unregister(perform_dev);
	class_destroy(perform_class);
}
...
//ljm for display sharing test
//
int set_display_data(void * data)
{
    return display_sharing_data->shmdata[0];
}
EXPORT_SYMBOL_GPL(set_display_data);

int set_fb_addr(dma_addr_t paddr,void *vaddr, __u32 handle, __u32 size,int index)
{
    //set fb vaddr, paddr, handle
    hyp_fb[index]->paddr = paddr;
    hyp_fb[index]->vaddr = vaddr;
    hyp_fb[index]->handle = handle;
    hyp_fb[index]->len = size;
    printk("ljm fb[%d] paddr %llx vaddr %p handle %d size %x \n",index,paddr,vaddr,handle,size);
    return 0;
}
EXPORT_SYMBOL_GPL(set_fb_addr);
int set_fb_id_by_handle(__u32  handle,uint32_t fb_base_id)
{
    //set fb id by handle, to match it
    int i = 0;
    for (i = 0; i <2; i++){
        if(hyp_fb[i]->handle == handle){
            hyp_fb[i]->fbid = fb_base_id;
//            printk("ljm  hyp_fb[%d] fbid %d handle %d",i,fb_base_id,handle);
            return 0;
        }
    }

    if(i >=2 ){
       printk("ljm something wrong, not fb handle %d\n",handle);
    }
    return 0;
}
EXPORT_SYMBOL_GPL(set_fb_id_by_handle);
int set_fb_id_by_paddr(dma_addr_t  paddr,uint32_t fb_base_id)
{
    //set fb id by handle, to match it
    int i = 0;
    for (i = 0; i <2; i++){
        if(hyp_fb[i]->paddr == paddr){
            hyp_fb[i]->fbid = fb_base_id;
//            printk("ljm  hyp_fb[%d] fbid %d paddr %llx",i,fb_base_id,paddr);
            return 0;
        }
    }

    if(i >=2 ){
       printk("ljm something wrong, not fb paddr %llx\n",paddr);
    }
    return 0;
}
EXPORT_SYMBOL_GPL(set_fb_id_by_paddr);

int set_frontfb_id(uint32_t fb_base_id)
{
    //set fb id by handle, to match it
    int i = 0;
    for (i = 0; i <2; i++){
        if(hyp_fb[i]->fbid == fb_base_id){
            hyp_fb[i]->isfront = 1;
//            printk("ljm  hyp_fb[%d] fbid %d is now front",i,hyp_fb[i]->fbid);
        }else{
            hyp_fb[i]->isfront = 0;
//            printk("ljm hyp_fb[%d] fbid %d is now back",i,hyp_fb[i]->fbid);
        }
    }
/*
    if(i >=2 ){
        //this is not framebuffer address
       printk("ljm something wrong, not fb id %d\n",fb_base_id);
    }
    */
    display_sharing_data->ctrl->notify = ~0;
    return 0;
}
EXPORT_SYMBOL_GPL(set_frontfb_id);


int set_frontfb_by_paddr(dma_addr_t paddr)
{
    //set fb id by handle, to match it
    int i = 0;
    for (i = 0; i <2; i++){
        if(hyp_fb[i]->paddr == paddr){
            hyp_fb[i]->isfront = 1;
//            printk("ljm  hyp_fb[%d] fbid %llx is now front",i,hyp_fb[i]->paddr);
        }else{
            hyp_fb[i]->isfront = 0;
//            printk("ljm hyp_fb[%d] fbid %llx is now back",i,hyp_fb[i]->paddr);
        }
    }
/*
    if(i >=2 ){
        //this is not framebuffer address
       printk("ljm something wrong, not framebuffer paddr %llx\n",paddr);
    }
    */
//after framebuffer swap. we need to notify the qnx side
    display_sharing_data->ctrl->notify = ~0;
    return 0;
}
EXPORT_SYMBOL_GPL(set_frontfb_by_paddr);

int is_fb_handle(__u32 handle)
{
    //check if this handle is framebuffer handle.
    int i = 0;
    for (i = 0; i <2; i++){
    if(hyp_fb[i]->handle == handle)
        return 1;
...
EXPORT_SYMBOL_GPL(is_fb_handle);
int is_fb_id(__u32 fb_id)
{
    //check if this handle is framebuffer handle.
    int i = 0;
    for (i = 0; i <2; i++){
    if(hyp_fb[i]->fbid == fb_id)
        return 1;
    }
        return 0;
}
EXPORT_SYMBOL_GPL(is_fb_id);

int is_fb_paddr(dma_addr_t paddr)
{
    //check if this handle is framebuffer handle.
    int i = 0;
    for (i = 0; i <2; i++){
