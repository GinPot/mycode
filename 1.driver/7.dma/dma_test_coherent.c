...
static int dma_test_init(void)
{
	...
	src = dma_alloc_coherent(driverDev, DMA_DATA_LEN, &dma_src, GFP_KERNEL);				//直接申请nocache的内存
	if(!src){
		pr_err("%s: src failed to allocate %d bytes of coherent memory for hardware descriptors.\n", __func__, DMA_DATA_LEN);
		ret =  -ENOMEM;
		goto err_src;
	}
	memset(src, 0x88, DMA_DATA_LEN);
	
	dst = dma_alloc_coherent(driverDev, DMA_DATA_LEN, &dma_dst, GFP_KERNEL);
	if(!dst){
		pr_err("%s: dst failed to allocate %d bytes of coherent memory for hardware descriptors.\n", __func__, DMA_DATA_LEN);
		ret =  -ENOMEM;
		goto err_dst;
	}
	...
	init_completion(&rx_dma_complete);														//初始化等待队列
...
err_dma:
	if(src)
		dma_free_coherent(driverDev, DMA_DATA_LEN, src, dma_src);
err_dst:
	if(dst)
		dma_free_coherent(driverDev, DMA_DATA_LEN, dst, dma_dst);
err_src:
