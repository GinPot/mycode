#ifndef _SOCKET_FRAME_CLIENT_H
#define _SOCKET_FRAME_CLIENT_H
...
public:
    SocketFrameClient(int sock, bool owned);
    SocketFrameClient(int sock, bool owned, bool useCmdNum);
    virtual ~SocketFrameClient();
    ...
    // SocketListener creates a SocketFrameClient (at refcount 1) and calls
    // decRef() when it's done with the client.
    void incRef();
    bool decRef(); // returns true at 0 (but note: SocketFrameClient already deleted)
...
typedef android::sysutils::List<SocketFrameClient *> SocketClientCollection;
#endif
