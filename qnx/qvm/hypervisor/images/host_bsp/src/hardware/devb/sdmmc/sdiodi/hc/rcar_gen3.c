...
static int rcar_sdmmc_intr_event(sdio_hc_t *hc)
{
    ...
    if (stat2 & (SDH_INFO2_ALL_ERR)) {
        ...
        sdio_slogf(_SLOGC_SDIODI, _SLOG_ERROR, hc->cfg.verbosity, 1,
                        ...
                        __func__, cmd->opcode, ests1, ests2);
        ...
        if ((sdmmc->flags & OF_DMA_ACTIVE) && (cmd->flags & SCF_CTYPE_ADTC)) {
            ...
            /*
             * Reset DMA channels on:
             *  1. DMA errors
             *  2. CMD_CRC_ERR (channels would stop responding)
             */
            if((dma_info2 & (DM_INFO2_DTRAN_ERR0 | DM_INFO2_DTRAN_ERR1)) || (ests1 & ((1 << 8) | (1 << 9)))) {
                sdio_slogf( _SLOGC_SDIODI, _SLOG_ERROR, hc->cfg.verbosity, 1, "%s: CMD%d DMA error DMA info1: 0x%X DMA info2 0x%X ERR_STS1=0x%X",
                    __func__, cmd->opcode, dma_info1, dma_info2, ests1 );
...
static int rcar_sdmmc_clk(sdio_hc_t *hc, int clk)
{
    ...
    /* disable timeouts */
    sdmmc_write(sdmmc->vbase, MMC_SD_OPTION, sdmmc_read(sdmmc->vbase, MMC_SD_OPTION) | SDH_OPTION_DISABLE_TO);
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/mainline/hardware/devb/sdmmc/sdiodi/hc/rcar_gen3.c $ $Rev: 836542 $")
#endif
