...
rcar_config h3_ws11_config[] = {
    ...
    [DEV_DISPLAY] = {
        ...
        .mod = (rcar_module[])
        {
            ...
            { .clk.val = ENABLE, .clk.reg = SMSTPCR(6), .clk.bits = BIT(29) }, /* VSPI2 */
            ...
            { .clk.val = ENABLE, .clk.reg = SMSTPCR(6), .clk.bits = BIT(0)  }, /* FCPVD3 */
            { { 0 } }
        },
        .cpg = (rcar_pin[])
        {
...
rcar_config h3_ws20_config[] = {
    ...
    [DEV_USBH] = {
        .mod = (rcar_module[])
        {
            ...
            { .clk.val = ENABLE,  .clk.reg = SMSTPCR(7), .clk.bits = BIT(2), .base = RCAR_GEN3_USB1_BASE }, /* EHCI0/OHCI1 */
            { .clk.val = ENABLE,  .clk.reg = SMSTPCR(7), .clk.bits = BIT(1), .base = RCAR_GEN3_USB2_BASE }, /* EHCI0/OHCI2 */
            { .clk.val = DISABLE, .clk.reg = SMSTPCR(7), .clk.bits = BIT(0) },                              /* EHCI0/OHCI3 */
            { { 0 } }
        },
        .blk_config = config_usbh,
    },
    ...
    [DEV_USB3D] = {
        .mod = (rcar_module[])
        {
            ...
            { .clk.val = DISABLE, .clk.reg = SMSTPCR(7), .clk.bits = BIT(5)  }, /* HS-USB-IF3 */
            { { 0 } }
        },
        .pinmux = (rcar_pinmux[])
        {
            { .gpsr.reg = GPSR(6), .gpsr.bits = BIT(30), .gpsr.val = GPSR_IP, .ipsr.reg = IPSR(18), .ipsr.bits = 0, .ipsr.val = 0 }, /* USB3_PWEN */
            { .gpsr.reg = GPSR(6), .gpsr.bits = BIT(31), .gpsr.val = GPSR_IP, .ipsr.reg = IPSR(18), .ipsr.bits = 4, .ipsr.val = 0 }, /* USB3_OVC */
            { { 0 } }
        }
    },
    [DEV_MSIOF] = {
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/mainline/hardware/startup/boards/rcar_gen3/rcar_h3/rcar_soc_board.h $ $Rev: 850154 $")
#endif
