...
rcar_i2c_init(int argc, char *argv[])
{
    ...
    if (!dev->physbase || !dev->irq) {
        rcar_i2c_slogf(dev, VERBOSE_QUIET, "%s: missing physbase or irq", __FUNCTION__);
        goto fail;
    }
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/mainline/hardware/i2c/rcar-A/init.c $ $Rev: 836822 $")
#endif
