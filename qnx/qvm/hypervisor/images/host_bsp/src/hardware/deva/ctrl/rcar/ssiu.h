...
/* one SSI channel set-up for independent transfer:
   ...
   tdm_exsplit_map         - channel map for TDM exsplit mode, indicates number of channels
                             mapped to each busif interface (ssi subchannel);
                             NULL if TDM exsplit is not used; points to an array of size four
                             if SSI_OP_MODE_TDMEXSPLIT_8CHAN is used, or an array of size eight
                             if SSI_OP_MODE_TDMEXSPLIT_16CHAN is used
*/
...
/* two SSI channels set-up for duplex transfer in pin sharing mode:
   ...
   ssi_channel_rx             - SSI channel to use for receive
   op_mode                    - operation mode to use
   ...
   voicenum_ext               - num voices on the external serial sound interface (SSI) bus, different
                                from voicenum only when extended TDM mode is used
   config                     - audio format config
   tdm_exsplit_map_tx         - channel map for TDM exsplit mode for transmit, indicates number
                                of channels mapped to each busif interface (ssi subchannel);
                                NULL if TDM exsplit is not used; points to an array of size four
                                if SSI_OP_MODE_TDMEXSPLIT_8CHAN is used, or an array of size eight
                                if SSI_OP_MODE_TDMEXSPLIT_16CHAN is used
   tdm_exsplit_map_rx         - channel map for TDM exsplit mode for receive, indicates number
                                of channels mapped to each busif interface (ssi subchannel);
                                NULL if TDM exsplit is not used; points to an array of size four
                                if SSI_OP_MODE_TDMEXSPLIT_8CHAN is used, or an array of size eight
                                if SSI_OP_MODE_TDMEXSPLIT_16CHAN is used
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/mainline/hardware/deva/ctrl/rcar/ssiu.h $ $Rev: 833552 $")
#endif
