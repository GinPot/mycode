...
int audio_dmac_init(audio_dmac_context_t* tx_context, audio_dmac_context_t* rx_context)
{
    ...
    if( tx_context ) {
        ...
        {
            ado_error_fmt( "unable to attach to audio DMA playback channel" );
            ret = EAGAIN;
            goto dmac_init_fail;
        }
    ...
    if( rx_context ) {
        ...
        {
            ado_error_fmt( "unable to attach to audio DMA capture channel" );
            ret = EAGAIN;
            goto dmac_init_fail;
        }
    ...
    if( tx_context ) {
        ...
        if( !tx_context->audiodma_pp_chn ) {
            ado_error_fmt( "unable to attach to audio pp DMA playback channel" );
            ret = EAGAIN;
            goto dmac_init_fail;
        }
    }
    ...
    if( rx_context ) {
        ...
        if( !rx_context->audiodma_pp_chn ) {
            ado_error_fmt( "unable to attach to audio pp DMA capture channel" );
            ret = EAGAIN;
            goto dmac_init_fail;
        }
    }
...
void audio_dmac_init_cleanup(audio_dmac_context_t* tx_context, audio_dmac_context_t* rx_context)
{
    ...
    if ( tx_context && tx_context->audiodma_pp_chn ) {
        audiodmappfuncs.channel_release( tx_context->audiodma_pp_chn );
    }
    if ( rx_context && rx_context->audiodma_pp_chn ) {
        audiodmappfuncs.channel_release( rx_context->audiodma_pp_chn );
    }
}
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/mainline/hardware/deva/ctrl/rcar/dmac.c $ $Rev: 850894 $")
#endif
