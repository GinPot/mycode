...
int ravb_process_interrupt (void *arg, struct nw_work_thread *wtp)
{
    ...
    if (iss & (ISS_FRS | ISS_FTS | ISS_TFUS))
    {
        ...
        if (iss & ISS_FRS) /* Frame received Mirror */
        {
            if (ris0 & RIS0_FRF0)
            {
                out32(ravb->base + RIS0, ~RIS0_FRF0);
                ravb_receive(ravb, wtp, 0);
            }
            if (ris0 & RIS0_FRF1)
            {
                out32(ravb->base + RIS0, ~RIS0_FRF1);
        ...
        if (iss & ISS_FTS) /* Frame transmitted Mirror */
        {
            ...
            if (tis & TIS_FTF1)
            {
                out32(ravb->base + TIS, ~TIS_FTF1);
            }
        }

        /* TimeStamp Updated Interrupt */
        if (iss & (ISS_TFUS | ISS_TFWM)) /* Time Stamp FIFO Interrupt */
        {
            /* Timestamp updated */
            if (tis & TIS_TFUF)
            {
                out32(ravb->base + TIS, ~TIS_TFUF);
            }
            /* Timestamp level Warning */
            if (tis & TIS_TFWF)
            {
                out32(ravb->base + TIS, ~TIS_TFWF);
            }
...
int ravb_dmac_enable_interrupt (void * arg)
{
    ...
    /* Frame transmitted, Timestamp FIFO */
    out32(ravb->base + TIC, TIC_FTE0 | TIC_FTE1 | TIC_TFUE | TIC_TFWE );
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/mainline/hardware/devnp/ravb/event.c $ $Rev: 837056 $")
#endif
