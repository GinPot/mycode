...
int clk_5p49_config(int du_index, int source_clk)
{
	...
	switch(du_index)
	{
		...
		case DU_CH_2:/* config CLK_OUT2 route to du_dotclkin2 */
			if((pwr_base = mmap_device_io(8, 0xFEB50004)) == (uintptr_t)MAP_FAILED){
				printf("mmap_device_io fail\n");
			}
			out32(pwr_base,0x400000);
			//printf("vaule=%x\n",in32(pwr_base));
			munmap_device_io(pwr_base, 8);
		...
		case DU_CH_3:/* config CLK_OUT2 route to du_dotclkin3 */
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/graphics/branches/dev-renesas-hypervisor/hardware/wfd/rcar3/wfdcfg/salvator-x-m3/clk-5p49v5923a.c $ $Rev: 846899 $")
#endif
