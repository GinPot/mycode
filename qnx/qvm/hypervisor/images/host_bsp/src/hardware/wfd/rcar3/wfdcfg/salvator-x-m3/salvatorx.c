...
static const struct mode lvds_modes[] = {
	{
		...
		.flags = WFDCFG_INVERT_VSYNC | WFDCFG_INVERT_HSYNC | WFDCFG_PREFERRED, // Mark this native resolution
		},
...
static const struct mode hdmi_modes[] = {
	...
	{
		.timing = {  // 1280x800 @ 60 Hz
			.pixel_clock_kHz = 74285,
			.hpixels = 1280, .hbp = 100, .hsw = 40, .hfp = 82,  // 1502 total
			.vlines = 800,   .vbp = 14,  .vsw = 6,  .vfp = 4,   //  824 total
			.flags = WFDCFG_INVERT_VSYNC | WFDCFG_INVERT_HSYNC,
		},
		.ext = external_fixed_clk_portmode_ext,
	},
	{
		.timing = {  // 1920x720 @ 60 Hz
			.pixel_clock_kHz = 114820,
			.hpixels = 1920, .hfp = 60, .hsw = 20, .hbp = 80, // 2080 total
			.vlines  = 720, .vfp =  2, .vsw =  1, .vbp =  197, // 920 total
			.flags = WFDCFG_INVERT_VSYNC | WFDCFG_INVERT_HSYNC | WFDCFG_PREFERRED, // Mark this native resolution
		},
		.ext = external_fixed_clk_portmode_ext,
	},	
...
static const struct mode vga_modes[] = {
	{
		.timing = {  // 1920x1080 @ 60 Hz
			.pixel_clock_kHz = 148500,
			.hpixels = 1920, .hfp = 88, .hsw = 44, .hbp = 148, // 2200 total
			.vlines  = 1080, .vfp =  4, .vsw =  5, .vbp =  36, // 1125 total
			.flags = WFDCFG_INVERT_VSYNC | WFDCFG_INVERT_HSYNC, // Mark this native resolution
		},
		.ext = external_configurable_clk_portmode_ext,
	},	
	{
		...
		const struct wfdcfg_keyval *opts)
{
	...
	if (key) {
		...
		case DU_CH_1:
			...
			break;
		case DU_CH_2:       /* For R-Car M3W */
		case DU_CH_3:       /* For R-Car M3N */
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/graphics/branches/dev-renesas-hypervisor/hardware/wfd/rcar3/wfdcfg/salvator-x-m3/salvatorx.c $ $Rev: 847961 $")
#endif
