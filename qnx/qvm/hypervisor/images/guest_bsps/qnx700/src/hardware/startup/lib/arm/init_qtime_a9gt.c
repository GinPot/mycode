...
timer_start_a9gt()
{
	...
	tmp = in32(mpcore_scu_base + GTIM_CONTROL);		// GT control register
	out32(mpcore_scu_base + GTIM_CONTROL, tmp & ~0xf);	// disable counter
	out32(mpcore_scu_base + GTIM_COUNTERL, ~0);		// set maximum count
	out32(mpcore_scu_base + GTIM_CONTROL, tmp | 1);		// enable counter

	return in32(mpcore_scu_base + GTIM_COUNTERL);		// counter
}

static unsigned
timer_diff_a9gt(unsigned start)
{
	unsigned now = in32(mpcore_scu_base + GTIM_COUNTERL);	// counter
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/mainline/hardware/startup/lib/arm/init_qtime_a9gt.c $ $Rev: 844419 $")
#endif
